...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>主  循  环<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    while(ros::ok())
    {
        ...
        case prometheus_msgs::ControlCommand::Land:
            ...
            {
                // 紧急降落
                cout << "quick land" << endl;
                state_sp << 0.0, 0.0, -Land_speed;
                yaw_sp = _DroneState.attitude[2]; //rad
                _command_to_mavros.send_vel_setpoint(state_sp,yaw_sp);
            }
            else if(Land_mode == 1)  // static landing
            {
                // root1 tests
                cout << "land_mode = 1" << endl; 
                cout << "last mode is " << unsigned(Command_Last.Mode) << endl;
                if (Command_Last.Mode != prometheus_msgs::ControlCommand::Land)  // static landing
                {
                    // root1 tests
                    cout << "last mode is not land" << endl;
                ...
                if(_DroneState.position[2] <= Disarm_height)  // static landing: at last
                {
                    // root1 tests
                    cout << "current height of the drone is " << _DroneState.position[2] << endl;
                    cout << "pos_z <= disarm_height"  << endl;
                    if(_DroneState.mode != "AUTO.LAND") // 无效
                    {
                        // root1 tests
                        cout << "force auto land" << endl;
                        //此处切换会manual模式是因为:PX4默认在offboard模式且有控制的情况下没法上锁,直接使用飞控中的land模式
                        _command_to_mavros.mode_cmd.request.custom_mode = "AUTO.LAND";
                        _command_to_mavros.set_mode_client.call(_command_to_mavros.mode_cmd);
                        pub_message(message_pub, prometheus_msgs::Message::WARN, NODE_NAME, "LAND: inter AUTO LAND flight mode");
                    }
                }
                else if(_DroneState.position[2] > Disarm_height)  // static landing: at first
                {
                    cout << "current height of the drone is " << _DroneState.position[2] << endl;
                    cout << "pos_z > disarm_height" << endl;
                    Command_Now.Reference_State.position_ref[0] = _DroneState.position[0];  // root1: 注释掉这两句?
                    Command_Now.Reference_State.position_ref[1] = _DroneState.position[1];
                    cout << "pos_ref_x is " << Command_Now.Reference_State.position_ref[0] << endl;
                    cout << "pos_ref_y is" << Command_Now.Reference_State.position_ref[1] << endl;
                   ...
                   //  _command_to_mavros.send_pos_vel_xyz_setpoint(state_sp,state_sp_extra,yaw_sp);
                    _command_to_mavros.send_vel_setpoint(state_sp_extra,yaw_sp);
                }
                
                if(_DroneState.landed)  // static landing
                {
                    // root1 tests
                    cout << "drone state landed" << endl;
                    Command_Now.Mode = prometheus_msgs::ControlCommand::Idle;
                }
            }
            else  // land mode != 1
            {
                // root1 tests
                cout << "land mode != 1" << endl;
                if (Command_Last.Mode != prometheus_msgs::ControlCommand::Land)
                {
                    // root1 tests
                    // cout << "the last mode is not land" << endl;
                    Command_Now.Reference_State.Move_mode       = prometheus_msgs::PositionReference::XYZ_POS;
                    Command_Now.Reference_State.Move_frame      = prometheus_msgs::PositionReference::ENU_FRAME;
                    Command_Now.Reference_State.position_ref[0] = _DroneState.position[0];
                    Command_Now.Reference_State.position_ref[1] = _DroneState.position[1];
                ...
                {
                    // root1 tests
                    cout << "position[2] > disarm_height" << endl;
                    Command_Now.Reference_State.position_ref[2] = _DroneState.position[2] - Land_speed * dt ;
                    Command_Now.Reference_State.velocity_ref[0] = 0.0;
                    Command_Now.Reference_State.velocity_ref[1] =  0.0;
                    Command_Now.Reference_State.velocity_ref[2] = - Land_speed; //Land_speed
                ...
                {
                    // root1 tests
                    cout << "position <= disarm_height" << endl;
                    //此处切换会manual模式是因为:PX4默认在offboard模式且有控制的情况下没法上锁,直接使用飞控中的land模式
...
int check_failsafe()
{
    ...
    {
        // root1 debug on 301023
        // cout << RED << "drone position " << _DroneState.position[0] << ", " << _DroneState.position[1] << ", "<< _DroneState.position[2]<<TAIL << endl;
        // cout << RED << "geo fence " << geo_fence_x[0] << ", " << geo_fence_y[0] << ", " << geo_fence_z[0] << TAIL << endl;
        // cout << RED << "geo fence " << geo_fence_x[1] << ", " << geo_fence_y[1] << ", " << geo_fence_z[1] << TAIL << endl;
