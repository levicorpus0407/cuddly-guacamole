...
#define NODE_NAME "planning_pbvs_landing"
...
// root1 adds on 201123, for velocity mux
std_msgs::Int8 flight_mode;
...
void groundtruth_cb(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    // 如果不让代码进vff也有问题 就是控制器的问题 可是只改了vt的计算部分 控制器完全没动 到底哪里有问题啊
    vff_received = true;
    target_twist = GroundTruth.twist.twist;
}
...
int main (int argc, char** argv)
{
	...
	//ROS Initialization
    ros::init(argc, argv, "planning_pbvs_landing");
    ...
    // root1 191123 changes the name of twist command of the drone, for velocity mux
    // root1 tests on 231223 no velocity mux
    ros::Publisher command_pub = nh.advertise<prometheus_msgs::ControlCommand>("/prometheus/control_command", 10);
    // ros::Publisher command_pub = nh.advertise<prometheus_msgs::ControlCommand>("/prometheus/landing/control_command", 10);
    ...
    // root1 adds on 191123, for velocity_mux
    // 【发布】飞行模式
    ros::Publisher flightMode_pub = nh.advertise<std_msgs::Int8>("/flight_mode_mux", 10);
    ...
    if(sim_mode)
    {
        // root1 adds on 201123, for test the landing node individually
        flight_mode.data = 1;
        flightMode_pub.publish(flight_mode);
        ...
        {
            // root1 adds on 201123
            // flight_mode.data = 1;
            // flightMode_pub.publish(flight_mode);

            // root1 comments on 201123
            ...
            // root1 changes on 191123
            // cout << "Switch to OFFBOARD and arm ..."<<endl;
            cout << "pbvs landing node starts" << endl;
    ...
    // root1 comments on 131123 do not need to take off
    // root1 uncomments it on 201123, to test the landing node individually
    ...
    while (ros::ok())
    {
        ...
        switch (exec_state)
        {
            ...
            case WAITING_RESULT:
            {
                // root1 adds on 161123
                cout << "PBVS - WAITING_RETULT" << endl;
            ...
            {
                //  root1 adds on 161123
                cout << "PBVS - TRACKING" << endl;
                // system("rosnode kill /local_planner_main");
                ...
                */  
                ...
                {
                    cout << "vff_recieved" << endl;
                    // root1 edits on 2510 use the world frame in gazebo because the velocity twist of the vehicle is not constant anymore
                    // old version:
                    ...
                    // current version:
                    // target twist: world frame --> drone body frame
                    // root1 edits on 261023
                    // change the target twist(ground truth in gazebo) from the world frame/enu frame to the drone body frame
                   ev[0] = target_twist.linear.x;
                   ev[1] = target_twist.linear.y;
                   ev[2] = target_twist.linear.z;
                   ev[3] = target_twist.angular.x;
                   ev[4] = target_twist.angular.y;
                   ev[5] = target_twist.angular.z;

                   bRe = eRb.t();
                   vpVelocityTwistMatrix bWe(bRe); 
                   bvt = bWe * ev;
                   cout << "bRe = " << bRe << endl;
               ...
               // cout << "cRcd1 = " << cRcd1 << endl;
                ...
                // root1 tests on 311023
                // remember to delete 经过排查 初步估计是因为控制器某个部分的值被覆盖了 导致wz without vt恒为0
                vt[0] = 0.0;
                vt[1] = 0.0;
                vt[2] = 0.0;
                vt[3] = 0.0;
               ...
               vsError_pub.publish(vs_error);
                ...
                //Publish 
                // root1 modifies on 191123 comments two lines below. not sure about the hold mode, just publish the command anyway
                if (!hold_mode)
                   command_pub.publish(Command_Now);
                // command_pub.publish(Command_Now);
