...
{
    ros::init(argc, argv, "landpad_det2");
    ros::NodeHandle nh("~");
    ...
    id_pub = nh.advertise<std_msgs::Int8>("/vs/id", 10);
    ...
    // double last_tux, last_tuy, last_tuz;
    ...
    while (ros::ok())
    {
        ...
        if (markerids.size() > 0)
        {
            ...
            int t = -1;
            // root1 tests 1234 comments 19 43
            ...
            if (-1 != t)
            {
                ...
                // root1 adds on 190623
                // rotation vectors
                // tux = rvec.ptr<double>(0)[0];
                // tuy = rvec.ptr<double>(0)[1];
                // tuz = rvec.ptr<double>(0)[2];
                ...
                // root1 adds
                std_msgs::Float32MultiArray cRm;  // dim.cRm = 9 containing rotation matrix from the target frame to the camera frame
                ...
                for(int i=0; i<3; i++)
                {
                    for(int j=0; j<3; j++)
                    {
                        cRm.data.push_back(RoteM.ptr<double>(i)[j]);
                        pose_now.rot_cRm[k] = RoteM.ptr<double>(i)[j];
                        k++;
                    } 
                }
                ...
                // root1 adds
                // for(int i=0; i<3; i++)
                    // cRtm.data.push_back(tvec.at<double>(i));

                transformation_pub.publish(cRm);
            ...
            if (-1 != t)
            {
                ...
                // pose_now.tu[1] = tuy;
                // pose_now.tu[2] = tuz;
                // last_tux = pose_now.tu[0];
                // last_tuy = pose_now.tu[1];
                // last_tuz = pose_now.tu[2];
            }
            else
            {
                pose_now.header.stamp = ros::Time::now();
                ...
                // root1 adds on 190623
                // pose_now.tu[0] = last_tux;
                // pose_now.tu[1] = last_tuy;
                // pose_now.tu[2] = last_tuz;
            }
        }
        else
        {
